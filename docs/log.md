...
## [2026-06-15] arch | Sipariş & Ödeme Mimarisi (ADR-019 + docs/siparis-odeme-mimarisi.md): güven/escrow ikilemi çözümü — Faz A Kapıda Ödeme (para akışı yok, lisans yok), Faz B iyzico Pazaryeri (escrow'u lisanslı PSP tutar, komisyon split, platform parayı custody etmez). Kendi escrow yasal değil → reddedildi. FSM + veri modeli + hexagonal PaymentGateway.
## [2026-06-15] doc | Lisans & Uyum Rehberi (docs/lisans-ve-uyum.md): şirket/vergi/ETBİS, ödeme (lisans PSP'de—bizde değil), KVKK/VERBİS, GIDA (5996—satıcı işletme kayıt/onay belgesi, kritik), büyüyünce ETK eşik/e-ticaret lisansı. Hukuki teyit notu.
## [2026-06-15] feat | Altyapı: satıcı GIDA işletme kayıt belge no alanı (uyum) — Store modeli/repo/DTO/form (M2 0005 canlıda) + orders.payment_method (COD altyapısı). feature:seller tam i18n (strings.xml). Kullanıcı: "uygulama altyapısını hazırla".
## [2026-06-15] feat | AAA tema overhaul (compose-expert skill): zengin M3 renk rolleri (canlı yeşil + amber aksan + katmanlı surface/outline) + yuvarlatılmış Shapes + görseller — Home hero gerçek foto (LoremFlickr, scrim'li wordmark) + kategori foto tile'ları. Kullanıcı: "baştan pro/AAA, görselli yap".
